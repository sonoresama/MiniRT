...
/*   Updated: 2023/11/30 18:30:29 by blerouss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vect	mult_mat_vect_u(t_vect *vector, t_system system)
{
	...
	vector->y = system.x.y * system.x.y * (1 - cos(0.2618)) + cos(0.2618)
		+ system.x.y * system.x.z * (1 - cos(0.2618)) - system.x.x * sin(0.2618);
	vector->z = system.x.y * system.x.z * (1 - cos(0.2618)) + system.x.x * sin(0.2618)
		+ system.x.z * system.x.z * (1 - cos(0.2618)) + cos(0.2618);
	if (vector->y == -1)
	{
		vector->x = tmp.x;
		vector->y = tmp.y;
...
t_vect	mult_mat_vect_d(t_vect *vector, t_system system)
{
	...
	vector->x = system.x.x * system.x.x * (1 - cos(0.2618)) + cos(0.2618)
		+ system.x.x * system.x.y * (1 - cos(0.2618)) - system.x.x * sin(0.2618);
	vector->y = system.x.x * system.x.y * (1 - cos(0.2618)) + system.x.x * sin(0.2618)
		+ system.x.y * system.x.y * (1 - cos(0.2618)) + cos(0.2618);
	vector->z = system.x.z;
	if (vector->y == 1)
	{
		vector->x = tmp.x;
